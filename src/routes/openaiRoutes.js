...
function sendOpenAIPermissionDenied(res) {
...
function shouldInjectCodexImageGenerationTool(model) {
  return !(typeof model === 'string' && model.toLowerCase().endsWith('spark'))
}

function ensureCodexImageGenerationTool(body = {}, model = null) {
  if (!body || typeof body !== 'object' || !shouldInjectCodexImageGenerationTool(model)) {
    return body
  }
  ...
  const hasImageGenerationTool = body.tools.some((tool) => tool?.type === 'image_generation')
  if (!hasImageGenerationTool) {
    body.tools.push({ ...CODEX_IMAGE_GENERATION_TOOL })
  }
...
const handleResponses = async (req, res) => {
  ...
  try {
    ...
    if (!checkOpenAIPermissions(apiKeyData)) {
      ...
      return sendOpenAIPermissionDenied(res)
    }
    ...
    ensureCodexImageGenerationTool(req.body, schedulerModel)
...
function isMultipartImageRequest(req) {
  const contentType = req.headers?.['content-type'] || ''
  return contentType.toLowerCase().startsWith('multipart/form-data')
}

function getImageRequestSessionId(req) {
  const body = isMultipartImageRequest(req) ? {} : req.body || {}
  return (
    req.headers['session_id'] ||
    req.headers['x-session-id'] ||
    body.session_id ||
...
module.exports.handleImageEdit = handleImageEdit
